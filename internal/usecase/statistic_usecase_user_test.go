...
func TestStatisticUsecase_GetStatistics_VariousUserIDs(t *testing.T) {
	...
	seedProjectModulData(db, "user-1", 8, 15)
	seedProjectModulData(db, "user-2", 12, 25)
...
func TestStatisticUsecase_GetStatistics_MixedPermissions(t *testing.T) {
	...
	seedProjectModulData(db, "user-mixed-1", 15, 35)
	seedProjectModulData(db, "user-mixed-2", 20, 0)
...
func TestStatisticUsecase_ActualGetStatistics_WithRealEnforcer(t *testing.T) {
	...
	db := setupTestDB(t)
	seedTestDB(db, 15, 5)
	seedProjectModulData(db, "user-1", 10, 25)
