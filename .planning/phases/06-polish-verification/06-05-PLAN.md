...
<objective>
Regenerate Swagger documentation with all newly-annotated endpoints, audit and update .env.example with all configuration options from Phases 1-5, and create a memory verification checklist with attempted measurement.

Purpose: Complete the documentation deliverables for Phase 6 and establish the memory verification procedure.
Output: Complete Swagger docs, comprehensive .env.example, memory testing checklist with results (if environment permits).
</objective>
...
<task type="auto">
  <name>Task 3: Create memory verification procedure and attempt measurement</name>
  <files>.planning/phases/06-polish-verification/06-MEMORY-VERIFICATION.md</files>
  <action>
  Create a step-by-step memory verification procedure AND attempt to execute it if the environment allows.

  **Step 1: Create the checklist document** with these sections:
  ...
  **Step 2: Attempt actual measurement** (conditional execution):

  1. Check if the service can start: try `go build -o /tmp/invento-test cmd/app/main.go`
  2. If build succeeds, check if database is available by attempting to start the service with ENABLE_PPROF=true
  3. If the service starts successfully:
     - Run baseline heap capture via pprof HTTP endpoint
     - Create 5 test files of 5MB each using `dd if=/dev/urandom`
     - Attempt concurrent upload simulation using the documented curl commands
     - Capture peak heap measurement
     - Record all results in the MEMORY-VERIFICATION.md file under a "## Results" section
  4. If the service cannot start (missing DB, auth config, etc.):
     - Document the reason in a "## Results" section
     - Mark as: "Status: verified-by-procedure, pending environment access"
     - Record: "Build succeeded, service requires [missing dependency] to run. Procedure documented and ready for execution in a complete environment."

  This approach satisfies the roadmap requirement of "shows heap stays below 350MB" by attempting actual measurement, with a documented fallback if the environment does not support it.
  </action>
  <verify>Verify the file exists and contains all sections: `grep -c "##" .planning/phases/06-polish-verification/06-MEMORY-VERIFICATION.md` should show 7+ sections (including Results). If measurement was attempted, verify the Results section contains actual numbers. If not, verify it contains a clear status and reason.</verify>
  <done>Memory verification procedure documented with clear prerequisites, step-by-step instructions, and pass/fail criteria. Actual measurement attempted -- results recorded if environment allowed, otherwise status documented as "verified-by-procedure, pending environment access" with specific reason.</done>
</task>
...
5. Memory verification checklist exists with procedure AND results section (measured or documented as pending)
</verification>
...
- Memory verification procedure provides actionable steps with pass/fail criteria
- Memory measurement attempted; results recorded or environment limitation documented
- go build passes
</success_criteria>
